...
| 대상 | 규칙 |
|---|---|
| `G:\내 드라이브\여형준님\32 공부 자료\` | 원본. **읽기 전용.** 쓰기·이동·이름변경 전부 금지 |
| `C:\dev\yaksa-corpus\` | 산출물. 여기에만 씁니다 |
| git 원격 | **private 전용.** 공개 저장소 생성 금지 |

원본은 각 대학 정리본·연세대 배포용 D등급 리스트·국시원 기출입니다. 전부 저작물입니다.
원문 텍스트·이미지·추출 결과를 외부로 전송하거나 공개 저장소에 커밋하지 마세요.

`data/`, `render/`, `l1/`은 `.gitignore`에 넣고 스키마·스크립트·리포트만 추적합니다.
