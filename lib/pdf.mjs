...
/**
 * 한 쪽을 원시 PPM(P6) 으로. 회전해서 다시 굽기 위한 입력입니다.
 * pdftoppm 은 stdout 으로 PPM 을 안정적으로 내보내지 않아 임시 파일을 거칩니다.
 */
export async function pdfPageToPPM(file, page, { dpi = 150, tmpDir } = {}) {
  const root = path.join(tmpDir ?? os.tmpdir(), `ppm-${process.pid}-${page}-${Math.floor(performance.now())}`);
  await run(bin("pdftoppm"),
    ["-r", String(dpi), "-f", String(page), "-l", String(page), "-singlefile", file, root],
