...
/**
 * 원본 자료 폴더. 반드시 .env 의 SOURCE_ROOT 로 지정합니다.
 * 저장소는 공개이고 자료는 비공개라, 실제 경로를 코드에 넣지 않습니다.
 */
export const SOURCE_ROOT = process.env.SOURCE_ROOT || "";
if (!SOURCE_ROOT) {
  throw new Error(
    "SOURCE_ROOT 가 비어 있습니다.\n" +
    "  .env.example 을 .env 로 복사한 뒤 자료 폴더 경로를 넣으세요.",
  );
}
