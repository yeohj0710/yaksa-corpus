 ...
 */
export async function chat({ model, messages, schema, schemaName = "output", maxTokens = 8000, temperature = 0 }) {
  assertKey();
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    if (res.ok) {
      ...
      logUsage({ model, in: pt, out: ct, cached, tag: schemaName ?? "text" });
...
export async function vision({
  ...
  imageBase64, imagesBase64, schema, schemaName, maxTokens = 8000,
}) {
  ...
  return chat({ model, messages, schema, schemaName, maxTokens });
}
