...
function assertKey() {
  if (process.env.LLM_ENABLE !== "1") {
    const err = new Error(
      ...
      "  전사는 에이전트가 직접 하세요:  npm run agent:next\n" +
      "  꼭 API 로 돌려야 하면 사람이 직접 켭니다:\n" +
      "    LLM_ENABLE=1 node scripts/run-all.mjs --from l1-microbiology\n" +
      "  먼저 `node scripts/cost.mjs` 로 예상 비용부터 확인하세요.",
    );
    // 이게 없으면 runJobs 가 평범한 실패로 처리해서 6,210쪽을 하나씩 quarantine 합니다.
    // 차단은 즉시 전체를 멈춰야 합니다.
    err.stopPipeline = true;
    throw err;
  }
  if (!LLM.apiKey) {
    const err = new Error("LLM_API_KEY 가 비어 있습니다. .env.example 을 .env 로 복사해서 채우세요.");
    err.stopPipeline = true;
    throw err;
  }
}
