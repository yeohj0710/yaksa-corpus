...
import { pdfToPngAll, pdfPageToPPM, pageOf } from "./pdf.mjs";
import { ensureDir } from "./jobs.mjs";
import { detectDocRotation } from "./rotation.mjs";
import { ppmToRotatedPng } from "./image.mjs";
...
export async function ensureRendered(row) {
  ...
  const job = (async () => {
    ensureDir(dir);
    const file = absPath(row);
    // 눕혀진 문서는 세워서 굽습니다. 그대로 넣으면 모델이 못 읽고 내용을 지어냅니다.
    const deg = await detectDocRotation(file, row.pages || 1);
    if (deg === 0) {
      await pdfToPngAll(file, dir, { dpi: RENDER_DPI });
    } else {
      for (let p = 1; p <= (row.pages || 0); p++) {
        const ppm = await pdfPageToPPM(file, p, { dpi: RENDER_DPI });
        const png = ppmToRotatedPng(ppm, deg);
        writeFileSync(path.join(dir, `p-${String(p).padStart(4, "0")}.png`), png);
      }
      writeFileSync(path.join(dir, ".rotated"), String(deg), "utf8");
    }
